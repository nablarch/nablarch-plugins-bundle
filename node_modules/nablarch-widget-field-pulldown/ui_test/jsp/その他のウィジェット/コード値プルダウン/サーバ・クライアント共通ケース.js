...
function readonly属性のテスト() {
  ...
  var cases = test_for('readonly-test')
    ...
    , assertReadOnly = function (testcase, values) {
      ...
      equal($input.length, 1, testcase.testcase + "[input size]");
...
function cssClass属性のテスト() {
  ...
  equal(default_case.area.find('select').length, 1, default_case.testcase);
  equal(default_case.area.find('select.cssClass-test').length, 0, default_case.testcase);

  equal(blank_case.area.find('select').length, 1, blank_case.testcase);
  equal(blank_case.area.find('select.cssClass-test').length, 0, blank_case.testcase);

  equal(set_case.area.find('select.cssClass-test').length, 1, set_case.testcase);
}
