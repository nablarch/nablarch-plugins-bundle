...
function($){
  ...
  function 必須属性のみのテスト() {
    ...
    equal($node.length, 1, testcase.testcase + " 要素数が異なる(事前条件の確認)");
    doCancelTest();
  ...
  function 条件指定のテスト() {
    ...
    equal($node.length, 1, caseMsg + " 要素数の確認。");
  ...
  function 表示条件を別の要素を条件判定にするテスト() {
    ...
    // テストの事前条件の確認
    equal($trigger.length, 1, caseMsg + " triggerの要素数が異なる。");
    equal($condition.length, 1, caseMsg + " conditionの要素数が異なる。");
