...
function cssClass属性のテスト() {
  ...
  equal(default_case.area.find('input:radio').length, 3, default_case.testcase);
  equal(default_case.area.find('input:radio.cssClass-test').length, 0, default_case.testcase);

  equal(blank_case.area.find('input:radio').length, 3, blank_case.testcase);
  equal(blank_case.area.find('input:radio.cssClass-test').length, 0, blank_case.testcase);

  equal(set_case.area.find('input:radio.cssClass-test').length, 3, set_case.testcase);
}
...
function listFormat属性のテスト() {
  ...
  var cases = test_for('listFormat-test')
    ...
    , assertLabelFormat = function(testcase, formatSpec, label){ //確認画面ではformatの中にlabelのテキストがあることで検証する。
      ...
      equal($label.length, 1, testcase.testcase);
      equal(text, label, testcase.testcase);
    }
