...
<c:if test="${empty showTitle}">
  <n:set var="showTitle" value="true" scope="page"/>
</c:if>
...
<n:set var="tableKey" value="${(not empty resultSetName) ? resultSetName : resultNumName }" scope="page" />
<n:set var="tableId" value="${(not empty id) ? id : tableKey}" scope="page" />
<c:if test="${not empty tableId}">
  <n:set var="tableId" value="${fn:replace(tableId, '.', '_')}"  scope="page"/>
</c:if>
...
<listSearchResult:listSearchResult
  ...
  <jsp:attribute name="bodyRowFragment">
    ...
    <%-- 付加情報項目表示域 --%>
    <n:set var="additionalColumnInlayId" value="inlay_${tableId}_${count}" scope="page"/>
