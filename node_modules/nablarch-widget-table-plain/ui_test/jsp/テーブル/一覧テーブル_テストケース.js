  ...
  function resultNumName属性のテスト() {
     setup();
     var cases = test_for('resultNumName-test')
      ,  resultNumName = cases[0];

     equal($.trim(resultNumName.title.find(label).text()), "resultNumName属性", resultNumName.testcase + " [title label]");
     // ヘッダ行、付加情報用の行と合わせて7行が出力されること
     equal(resultNumName.table.find('tr').length, 7, resultNumName.testcase);
  }
