...
var pjConfName = path.basename(PJ_CONF_PATH, '.json');
var LAST_BUILD_FILE_LIST = path.resolve(path.join(UI_PLUGINS_PATH, pjConfName + 'LastBuildFileList.json'));
...
function main() {
  ...
  if (cleanMode) {
    ...
    // 通常モード
    removeLastBuildFile();
    // 今回ビルド対象のファイルを、事前に最後のビルドファイルリストに登録する
    // これで、途中でエラーでビルドが失敗した場合でも、次回実行時に前回ビルドファイルのクリーニングが確実に行われる。
      ...
      if (libraryDeployMappings[packageName]) {
        for(var srcKey in libraryDeployMappings[packageName]){
          ...
          LIB_DEPLOY_PATHS.each(function(deployPath) {
            var dst     = libraryDeployMappings[packageName][srcKey];
            var dstPath = path.join(deployPath, dst);
            var dstDir = path.dirname(dstPath);
            var dstFile = path.basename(dstPath);

            mkdir('-p', dstDir);
            copyFile(srcPath, dstDir, true);
            if (dst==srcKey) return; // 一緒なら無視。(fonts/font*) 対応
 ...
 * "use strict"を取り除く
 */
function removeUseStrict(targetFilePath) {
  var content = fs.readFileSync(targetFilePath, 'utf8');
  fs.writeFileSync(targetFilePath, content.replace(/['"]use strict['"];/g, ''));
}
...
function buildDevtoolJS() {
  ...
  BUILD_TARGET_PATHS.each(function(targetDir) {
    ...
    if (fs.existsSync(autoconfScript)) {
      ...
      // use strictを取り除く
      var devtoolConfDir = path.dirname(devtoolConf);
      var objDevtoolConf = JSON.parse(result.output);
      var devtooljsPath = path.resolve(devtoolConfDir, objDevtoolConf.out);
      removeUseStrict(devtooljsPath);
...
function buildMinifyJS() {
  ...
  BUILD_TARGET_PATHS.each(function(targetDir) {
    ...
    // use strictを取り除く
    removeUseStrict(minifyJsFileName);
...
function buildMinifyCSS() {
  ...
  BUILD_TARGET_PATHS.each(function(targetDir) {
    ...
    INTEGRATION_LESS_LIST.each(function(targetMode) {
      ...
      pushd(cssDir);
      execute(JS + ' ' + rScript + ' -o cssIn=' + cssFileName + ' out=' + minifyFileName + ' optimizeCss=default');
...
function execute(cmd, options) {
  var result = exec(cmd.replace(new RegExp(JS + ' '), JS + ' --no-deprecation '), options);
  if (result.code !== 0) {
