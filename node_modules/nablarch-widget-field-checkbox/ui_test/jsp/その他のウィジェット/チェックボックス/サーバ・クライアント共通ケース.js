  ...
  var cases = test_for('required-test')
    ...
    ,  assertHas    = function(testcase) {
      if(isConfirmationPage()) { assertNotHas(testcase);
        ...
        equal(testcase.area.find(required).length, 1, testcase.testcase);
        equal($.trim(testcase.area.find(required).text()), '*', testcase.testcase);
      }
    }
    , assertNotHas  = function(testcase) {
      equal(testcase.area.find(required).length, 0, testcase.testcase);
    };
...
function hint属性のテスト() {
  ...
  var cases = test_for('hint-test')
    ...
    , assertHasHint = function(testcase) {
      isConfirmationPage() ? equal(testcase.hint.length, 0, testcase.testcase)
        : equal(testcase.hint.length, 1, testcase.testcase);
    }
    , assertHasNotHint = function(testcase) {
      equal(testcase.hint.length, 0, testcase.testcase);
    };
...
function readonly属性のテスト() {
  ...
  var cases = test_for('readonly-test')
    ...
    , assertReadOnly = function(testcase, values) {
      ...
      equal($input.length, 3, testcase.testcase + "[input size]");
...
function cssClass属性のテスト() {
  ...
  equal(default_case.area.find('input:checkbox').length, 3, default_case.testcase);
  equal(default_case.area.find('input:checkbox.cssClass-test').length, 0, default_case.testcase);

  equal(blank_case.area.find('input:checkbox').length, 3, blank_case.testcase);
  equal(blank_case.area.find('input:checkbox.cssClass-test').length, 0, blank_case.testcase);

  equal(set_case.area.find('input:checkbox.cssClass-test').length, 3, set_case.testcase);
}
...
function listFormat属性のテスト() {
  ...
  var cases = test_for('listFormat-test')
    ...
    , assertValuesFormat = function(testcase, formatSpec, values) { // 入力画面ではformat内にcheckboxがあることで検証
      ...
      equal($input.length, values.length, testcase.testcase + " formatspec = " + formatSpec);
    ...
    , assertLabelFormat = function(testcase, formatSpec, labels){ //確認画面ではformatの中にlabelのテキストがあることで検証する。
      ...
      equal($labels.length, labels.length, testcase.testcase);
