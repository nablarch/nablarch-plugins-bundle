...
function 必須属性テスト() {
  ...
  if (isConfirmationPage()) {
    equal(target.input.text().trim(), '必須属性を指定', 'divのボディ部に入力値が表示されていること');
    equal(target.error.length, 0, 'エラー表示領域そのものが存在しないこと');
  } else {
    equal($.trim(target.input.find('textarea').val()), '必須属性を指定', 'textareaのbody部に値が出力できていること');
  ...
  var test = {
    nonNameAlias: function (testCase) {
      ...
      if (isConfirmationPage()) {
        ...
        equal(target.input.find('textarea.nablarch_error').length, 1, 'nablarch_errorクラスが設定されること');
...
function find_text(test_case) {
  var $test = $('span.test-case.' + test_case)
    , $text_div = $test.next('.field.textarea');
