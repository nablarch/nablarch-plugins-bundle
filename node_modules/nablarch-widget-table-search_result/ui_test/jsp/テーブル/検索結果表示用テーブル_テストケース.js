  ...
  function usePaging属性のテスト() {
    ...
    equal(default_case.table.find(paging_navi_area).length, 2, default_case.testcase);
    equal(blank_case.table.find(paging_navi_area).length, 2, blank_case.testcase);
    equal(true_case.table.find(paging_navi_area).length, 2, true_case.testcase);
    equal(false_case.table.find(paging_navi_area).length, 0, false_case.testcase);
  }
  ...
  function 必須属性のみ指定した場合のテスト() {
     ...
     equal(require_only.table.find(paging_navi_area).length, 0, require_only.testcase + "[paging navigate area size]");
